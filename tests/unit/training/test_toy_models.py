...
def test_toy_sparsity_limits():
    cfg = Config(n_instances=1)
    model = ReluOutputModel(cfg, feature_probability=0.0)
    batch = model.generate_batch(10)
    assert (batch == 0).all()

    model = ReluOutputModel(cfg, feature_probability=1.0)
    batch = model.generate_batch(10)
...
def test_toy_correlated_features(n_correlated_pairs: int):
    cfg = Config(
        n_instances=1, n_features=6, n_hidden=2, n_correlated_pairs=n_correlated_pairs
    )
    model = ReluOutputModel(cfg, feature_probability=0.5)
...
def test_reluoutput_forward():
    cfg = Config(n_instances=1, n_features=6, n_hidden=2)
    model = ReluOutputModel(cfg, feature_probability=0.5)
