...
class ActivationsStore(ABC):
    ...
    def get_activations(
        self, batch_tokens: torch.Tensor, extra_forward_kwargs: dict[str, Any] = {}
    ):
        """
        Returns activations of shape (batches, context, num_layers, d_in).
        d_in may result from a concatenated head dimension.

        In the case of toy models, returns (batches, num_layers, d_in).
        """
    ...
    def _load_activations_from_disk(self, buffer_size: int) -> torch.Tensor:
        d_in = self.d_in
...
class ToyActivationsStore(ActivationsStore):
    """
    Class for streaming tokens and generating and storing activations
    while training SAEs.
    """
    ...
    def get_batch_tokens(self) -> torch.Tensor:
        ...
        device = self.device
        batch_tokens = self.model.generate_batch(batch_size).squeeze().to(device)
        assert (
            len(batch_tokens.shape) == 2
        ), f"tokens.shape should be 2D but was {batch_tokens.shape}. Ensure toy model only has 1 instance."
    ...
    def get_buffer(self, n_batches_in_buffer: int) -> torch.Tensor:
        ...
        if self.cached_activations_path is not None:
            return self._load_activations_from_disk(buffer_size)
    ...
    def _get_next_dataset_tokens(self) -> torch.Tensor:
        tokens = self.model.generate_batch(1).squeeze()
        assert (
            len(tokens.shape) == 1
        ), f"tokens.shape should be 1D but was {tokens.shape}. Ensure toy model only has 1 instance."
        return tokens.to(self.device)


class LMActivationsStore(ActivationsStore):
    """
    Class for streaming tokens and generating and storing activations
    while training SAEs.
    """
    ...
    def get_activations(
        ...
        d_in may result from a concatenated head dimension.
        """
        hook_point_max_layer = max(self.hook_point_layers)
        extra_forward_kwargs["stop_at_layer"] = hook_point_max_layer + 1
        extra_forward_kwargs["prepend_bos"] = self.prepend_bos
        extra_forward_kwargs.update(self.model_kwargs)
        return super().get_activations(batch_tokens, extra_forward_kwargs)
